...
def build_dashboard():
    ...
    for thema, count in thema_30d:
        ...
        thema_cards += (
            ...
            'data-thema="' + thema + '" style="border-top:3px solid ' + k + ';">'
            '<div class="thema-count" style="color:' + k + ';">' + str(count) + '</div>'
    ...
    for a in recent:
        ...
        bs = sanitize(bron)
        k = THEMA_KLEUREN.get(thema, "#94a3b8")
        ds = toegevoegd[:10] if toegevoegd else ""
        artikel_rijen += (
            '<div class="artikel-rij" data-thema="' + thema + '" data-bron="' + bs + '">'
            '<span class="thema-badge" style="background:' + k + ';">' + thema + '</span>'
            '<a href="' + url + '" target="_blank" class="artikel-titel">' + ts + '</a>'
            '<span class="artikel-meta">' + bs + ' &middot; ' + ds + '</span></div>\n'
        )
...
.stat-box{background:white;border-radius:14px;padding:22px;text-align:center;box-shadow:0 1px 4px rgba(0,0,0,.08);border:1px solid #e2e8f0;}
...
.thema-card{background:white;border-radius:12px;padding:16px;cursor:pointer;transition:all .15s;box-shadow:0 1px 4px rgba(0,0,0,.06);border:1px solid #e2e8f0;}
.thema-card:hover{transform:translateY(-2px);}
.thema-card.active{background:#eff6ff;border-color:#2563eb;}
.thema-count{font-size:28px;font-weight:800;line-height:1;}
.thema-naam{font-size:12px;color:#64748b;margin-top:4px;}
.charts-grid{display:grid;grid-template-columns:1.5fr 1fr;gap:20px;margin-bottom:28px;}
.chart-box{background:white;border-radius:14px;padding:20px;box-shadow:0 1px 4px rgba(0,0,0,.08);border:1px solid #e2e8f0;}
.chart-filters{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:14px;}
.filter-chip{font-size:11px;font-weight:600;padding:3px 10px;border-radius:20px;cursor:pointer;border:1.5px solid #e2e8f0;background:white;color:#64748b;}
.artikels-box{background:white;border-radius:14px;padding:24px;box-shadow:0 1px 4px rgba(0,0,0,.08);border:1px solid #e2e8f0;}
...
<header><div class="header-inner"><div>
<h1>Dendermonde <span>Nieuws</span></h1>
<div style="color:#bfdbfe;font-size:13px;">Lokaal nieuwsoverzicht</div>
</div><div class="update-info">Laatste update<br><strong>""")
    html_parts.append(nu)
    html_parts.append("""</strong></div></div></header>
<div class="container">
<div class="stats-row">
...
var THEMA_KLEUREN = """)
    ...
    html_parts.append(""";
var ALLE_BRONNEN = """)
    html_parts.append(alle_bronnen_json)
    html_parts.append(""";
var actieveThemas = new Set();
var actieveBronnen = new Set();

var bronFilterEl = document.getElementById('bronFilter');
ALLE_BRONNEN.forEach(function(bron) {
    var chip = document.createElement('span');
    chip.className = 'filter-chip';
    chip.textContent = bron;
    chip.id = 'bron_' + bron.replace(/[^a-zA-Z0-9]/g, '_');
    chip.onclick = (function(b) { return function() { toggleFilter('bron', b); }; })(bron);
    bronFilterEl.appendChild(chip);
});

var weekFiltersEl = document.getElementById('weekFilters');
var weekFilterItems = [['Totaal', null]].concat(ALLE_BRONNEN.map(function(b) { return [b, b]; }));
weekFilterItems.forEach(function(item) {
    ...
    chip.onclick = (function(v) { return function() { setWeekFilter(v); }; })(val);
    weekFiltersEl.appendChild(chip);
});

var weken = Object.keys(WEEK_DATA);
var weekChart = new Chart(document.getElementById('weekChart'), {
    type: 'bar',
    data: {
        labels: weken,
        datasets: [{ label: 'Artikels', data: weken.map(function(w) { return WEEK_DATA[w].totaal; }), backgroundColor: '#3b82f6', borderRadius: 5 }]
    },
    options: { responsive: true, plugins: { legend: { display: false } },
        scales: { x: { ticks: { color: '#94a3b8', font: { size: 11 } }, grid: { color: '#f1f5f9' } },
...
new Chart(document.getElementById('bronChart'), {
    type: 'doughnut',
    data: { labels: """)
    html_parts.append(bron_labels_json)
    html_parts.append(""", datasets: [{ data: """)
    html_parts.append(bron_values_json)
...
function setWeekFilter(filter) {
    document.querySelectorAll('#weekFilters .filter-chip').forEach(function(c) {
        c.style.background='white'; c.style.color='#64748b'; c.style.borderColor='#e2e8f0';
    });
    var id = 'wf_' + (filter || 'totaal').replace(/[^a-zA-Z0-9]/g, '_');
...
function toggleFilter(type, value) {
    var set = type === 'thema' ? actieveThemas : actieveBronnen;
    var kleur = type === 'thema' ? (THEMA_KLEUREN[value] || '#2563eb') : '#475569';
    var elId = type === 'bron' ? 'bron_' + value.replace(/[^a-zA-Z0-9]/g, '_') : null;
    var cardEl = type === 'thema' ? document.querySelector('.thema-card[data-thema="' + value + '"]') : null;
    var chipEl = elId ? document.getElementById(elId) : null;
    if (set.has(value)) {
        set.delete(value);
...
function updateActiveTags() {
    ...
    actieveThemas.forEach(function(t) {
        var k = THEMA_KLEUREN[t] || '#2563eb';
        var span = document.createElement('span');
        span.className = 'active-tag';
        span.style.background = k;
        var label = document.createTextNode(t + ' ');
        var x = document.createElement('span');
        x.textContent = 'x';
        x.style.cursor = 'pointer';
        x.onclick = (function(val) { return function() { toggleFilter('thema', val); }; })(t);
    ...
    actieveBronnen.forEach(function(b) {
        ...
        x.textContent = 'x';
        x.style.cursor = 'pointer';
        x.onclick = (function(val) { return function() { toggleFilter('bron', val); }; })(b);
        span.appendChild(label);
        span.appendChild(x);
        el.appendChild(span);
    });
...
function filterArtikels() {
    var zoek = document.getElementById('zoek').value.toLowerCase();
    document.querySelectorAll('.artikel-rij').forEach(function(rij) {
        var titel = rij.querySelector('.artikel-titel').textContent.toLowerCase();
        var ok = (!zoek || titel.includes(zoek))
...
function resetFilters() {
    ...
    document.querySelectorAll('.thema-card').forEach(function(c) { c.classList.remove('active'); });
    document.querySelectorAll('.bron-filter .filter-chip').forEach(function(c) {
        c.style.background='white'; c.style.color='#64748b'; c.style.borderColor='#e2e8f0';
    });
    document.getElementById('activeFilters').innerHTML = '';
    document.querySelectorAll('.artikel-rij').forEach(function(r) { r.style.display=''; });
    setWeekFilter(null);
}
</script></body></html>""")
    ...
    with open("docs/index.html", "w", encoding="utf-8") as f:
        f.write(html)
    print("docs/index.html gegenereerd (" + str(len(html)) + " bytes)")
